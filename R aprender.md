...
- Mas enquanto que uma sequência criada com o operador ":" dá sempre numa sequência de número em incrementos = 1, com a  função seq() nós podemos alterar este incremento utilizando o argumento -> `seq(c(...), by = x)`.  
...
- Funções são um dos componentes fundamentais do R. São pequenas porções de código reutilizáveis.  
- Síntaxe para criar funções:  
...
- Dá para ver os argumentos de uma função utilizando a função `args()`
- Funções anónimas

```r
function(x){expressão}
```

- Função paste  

```r
paste("A","B","C","D") =  "A B C D"
```

- Argumentos não específicos
...
telegram("PORTUGAL", "YOUTUBE", "ARIZONA") ->  "START PORTUGAL YOUTUBE ARIZONA STOP"
```

- Operações binárias definidas pelo utilizador
